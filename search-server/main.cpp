...
public:
    ...
    void AddDocument(int document_id, const string& document, DocumentStatus status, const vector<int>& ratings) {
        ...
        //     if (!IsValidWord(word)) {
        //         throw invalid_argument("document text contains special sign!"s);
        //     }
        // }
        vector<string> words;
    ...
    vector<Document> FindTopDocuments(const string& raw_query, DocumentStatus status) const {
        return FindTopDocuments(raw_query,[status](int document_id, DocumentStatus target_status, int rating){
            return target_status == status;  //вот тут я забыл что происходит
        });
    }
    ...
    int GetDocumentId(int index) const {
        return documents_ids_.at(index); //??? give it a try
    }
    ...
    tuple<vector<string>, DocumentStatus> MatchDocument(const string& raw_query, int document_id) const {
        ...
        for(const string& query_word : query.plus_words) {
            ...
            if(word_to_document_freqs_.at(query_word).count(document_id)) {
                matched_words.push_back(query_word);
            }
        }
        for(const string& query_minus_word : query.minus_words) {
            if(word_to_document_freqs_.count(query_minus_word) == 0) {
                continue;
            }
            if(word_to_document_freqs_.at(query_minus_word).count(document_id)) {
                matched_words.clear();
                break;
            }
        }
...
private:
    ...
    vector<Document> FindAllDocuments(const Query& query, DocumentPredicate document_predicate) const {
        ...
        for(const string& word : query.plus_words) {
            ...
            const double word_idf = ComputeWordInverseDocumentFreq(word);
            for(const auto& [document_id, tf] : word_to_document_freqs_.at(word)) {
                ...
                if(document_predicate(document_id, document_data.status, document_data.rating)) {
                    document_to_relevance[document_id] += word_idf * tf;
                }
            }
        }
